    ...
    for propertyUID, property in pairs(allProperties) do
        if propertyUID and property then
            ...
            newCache[propertyUID] = {
                ...
                AvailableSpots = getAvailableSpotsCount(propertyUID),
                UID = propertyUID
            }
...
local function calculateRenterIncome(propertyUID, renter)
    if not propertyUID or not renter then return 0 end
...
-- Функция заполнения всех свободных мест
...
local function optimizeAllProperties()
    ...
    log(string.format("ЦИКЛ ОПТИМИЗАЦИИ #%d", cycleCount), "info")
    ...
    for propertyUID, property in pairs(allProperties) do
        if property and property.BuildingType and property.BuildingType ~= "Empty" then
            ...
            if result:find("filled|") then
                ...
                log(string.format("Заполнено %s мест %s", parts[2], parts[3]), "success")
    ...
    log(string.format("Время цикла: %.2f сек", statsData.cycleTime), "info")
...
local function startAutoOptimizer()
    if isRunning then
        log("Оптимизатор уже запущен", "warning")
        return
    end
...
-- Функция создания оптимизированного GUI
local function createOptimizedGUI()
    local PlayerGui = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
    
    -- Удаляем старый GUI если есть
    local oldGUI = PlayerGui:FindFirstChild("AutoOptimizerUI")
    ...
    StatusIndicator.Position = UDim2.new(1, -55, 0.5, -6)
    ...
    local function createStatItem(name, value, color, icon, position)
        local frame = Instance.new("Frame")
        frame.Size = UDim2.new(0.48, -5, 0, 28)
    ...
    local function createButton(text, color, callback)
        ...
        button.Size = UDim2.new(0.48, -5, 0, 45)
    ...
    local autoButton = createButton("АВТОРЕЖИМ", Color3.fromRGB(0, 180, 0), function()
        if not isRunning then
            task.spawn(startAutoOptimizer)
        end
    end)
    autoButton.Position = UDim2.new(0, 0, 0, 0)
    autoButton.Parent = ButtonsContainer
    
    local quickButton = createButton("БЫСТРАЯ", Color3.fromRGB(255, 150, 0), quickOptimize)
    quickButton.Position = UDim2.new(0.52, 5, 0, 0)
    quickButton.Parent = ButtonsContainer
    
    local fillButton = createButton("ЗАПОЛНИТЬ", Color3.fromRGB(0, 150, 255), forceFillAllSpots)
    fillButton.Position = UDim2.new(0, 0, 0, 50)
    fillButton.Parent = ButtonsContainer
    
    local stopButton = createButton("ОСТАНОВИТЬ", Color3.fromRGB(220, 0, 0), stopOptimizer)
    stopButton.Position = UDim2.new(0.52, 5, 0, 50)
    stopButton.Parent = ButtonsContainer
    
    -- Логи
    local LogCard = Instance.new("Frame")
    LogCard.Size = UDim2.new(1, -20, 0, 120)
